...
<script>
    ...
    var GoogleSatelliteHybrid = L.tileLayer('https://mt1.google.com/vt/lyrs=y&x={x}&y={y}&z={z}', {maxZoom: 22, attribution: 'Latihan Web SIG'}).addTo(map);
    ...
    var groupedOverlays = {
        ...
        "Peta Khusus":{
            'Kecamatan Mampang Prapatan': kecmam,
            'Indomart' :mart
        }
    };
    ...
    north.onAdd = function(map) {
        ...
        div.innerHTML = '<img src="<?=base_url()?>assets/compas.png"style=width:200px;>';
        return div; }
    north.addTo(map);

    $.getJSON("<?=base_url()?>assets/indomart.geojson",function(data){
            var ratIcon = L.icon({
            ...
            L.geoJson(data,{
                pointToLayer: function(feature,latlng){
                    ...
                    marker.bindPopup(feature.properties.Nama_Tempat);
                    return marker;
                }
            }).addTo(mart);
        });
    ...
    $.getJSON("<?=base_url()?>/assets/jaksel.geojson",function(id){
        L.geoJson( id, {
        style: function(feature){
            var fillColor,
            id = feature.properties.id;
            if ( id > 12 ) fillColor = "#006837";
            else if (id>11) fillColor="#1b7837"
            else if (id>10) fillColor="#d53e4f"
            else if (id>9) fillColor="#67001f"
            else if (id>8) fillColor="#c994c7"
            else if (id>7) fillColor="#fdbb84"
            else if (id>6) fillColor="#dd1c77"
            else if (id>5) fillColor="#3182bd"
            else if ( id > 4 ) fillColor ="#f03b20"
            else if ( id > 3 ) fillColor = "#31a354";
            else if ( id > 2 ) fillColor = "#78c679";
            else if ( id > 1 ) fillColor = "#c2e699";
            else if ( id > 0 ) fillColor = "#ffffcc";
            else fillColor = "#f7f7f7"; // no data
            return { color: "#999", weight: 1, fillColor: fillColor, fillOpacity: .6 };
        },
        onEachFeature: function( feature, layer ){
    ...
    $.getJSON("<?=base_url()?>/assets/kec_mampang.geojson",function(id){
        L.geoJson( id, {
        style: function(feature){
            var fillColor,
            id = feature.properties.id;
            if ( id > 2 ) fillColor = "#006837";
            else if ( id > 1 ) fillColor = "#c2e699";
            else if ( id > 0 ) fillColor = "#ffffcc";
            else fillColor = "#f7f7f7"; // no data
            return { color: "#999", weight: 1, fillColor: fillColor, fillOpacity: .6 };
